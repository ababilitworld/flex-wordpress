...
class Route
{
    public static function get(string $uri, array $action, string $label = '', string $capability = 'read'): void
    {
        RouteManager::getInstance()->register('GET', $uri, $action, $label, $capability);
    }

    public static function post(string $uri, array $action, string $label = '', string $capability = 'read'): void
    {
        RouteManager::getInstance()->register('POST', $uri, $action, $label, $capability);
    }

    public static function any(string $uri, array $action, string $label = '', string $capability = 'read'): void
    {
        RouteManager::getInstance()->register('ANY', $uri, $action, $label, $capability);
    }
}
...
class RouteManager
{
    ...
    public function register(string $method, string $uri, array $action, string $label = '', string $capability = 'read'): void
    {
        $this->routes[$uri] = compact('method', 'action', 'label', 'capability');
    }
...
class RouteHandler
{
    public static function dispatch(): void
    {
        ...
        if ($route && (strtoupper($_SERVER['REQUEST_METHOD']) === $route['method'] || $route['method'] === 'ANY')) {
            if (!current_user_can($route['capability'])) {
                wp_die(__('You do not have permission to access this route.', 'wp-route-manager'));
            }
...
class RouteModel
{
    ...
    public function __construct(string $method, string $uri, array $action, string $label = '', string $capability = 'read')
    {
        $this->method = $method;
        $this->uri = $uri;
        $this->action = $action;
